...
# Prefer an absolute, repo-relative local copy if available. If not present we
# leave DATA_PATH as None so the benchmark runner will download from HF.
candidate = _proj_root / "local_data" / "liander2024"
DATA_PATH = "local_data/liander2024-energy-forecasting-benchmark"
OUTPUT_PATH = Path("./benchmark_results")

BENCHMARK_RESULTS_PATH_XGBOOST = OUTPUT_PATH / "XGBoost"
BENCHMARK_RESULTS_PATH_GBLINEAR = OUTPUT_PATH / "GBLinear"
BENCHMARK_RESULTS_PATH_RF_LightGBM = OUTPUT_PATH / "RF_LightGBM"
...
rf_lightgbm_config = common_config.model_copy(update={"model": "rf_lightgbm"})
...
def _target_forecaster_factory(
    ...
    base_config = rf_lightgbm_config if context.run_name == "rf_lightgbm" else gblinear_config
