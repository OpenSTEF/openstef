...
# 2. Train a default XGBoost model and inspect the forecast, preprocessing and explainability.
# 3. Train an XGBoost + GBLinear ensemble with `openstef-meta` and compare its predictions
# 4. Forecast with Chronos-2 through `openstef-foundation-models`
# 5. Compare the output of the ensemble with Chronos-2
# 6. Run an optional short `openstef-beam` backtest
#
...
fig = px.line(
    profiles_df,
    title="Standard Consumption Profiles",
...
for transform in preprocessing_transforms:
    if hasattr(transform, "features_added"):
        if len(features_added := transform.features_added()) > 0:
...
features_to_plot = [
    ...
    ("gti", "Global Tilted Irradiance (gti) (W/m²)"),
    ("day_of_week_sine", "Day of Week (sine)"),
    ("time_of_day_sine", "Time of Day (sine)"),
]
window = input_features.loc["2024-02-17":"2024-03-08"]
...
# 2. Trained XGBoost
# 3. Built an ensemble with XGBoost and GBLinear through `openstef-meta`
# 4. Forecast with Chronos-2 through `openstef-foundation-models`
# 5. Compared the ensemble with Chronos-2
# 6. Backtested XGBoost, GBLinear, and Chronos-2 with `openstef-beam`
#
