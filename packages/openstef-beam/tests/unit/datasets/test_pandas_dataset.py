...
def sample_data() -> pd.DataFrame:
    """Create sample data with intentional patterns for testing."""
    data = {
        "timestamp": [
...
@pytest.fixture
def dataset(sample_data: pd.DataFrame) -> PandasTimeseriesDataset:
    """Create a PandasTimeseriesDataset with the sample data."""
...
def test_initialization_with_valid_data(sample_data: pd.DataFrame):
    ...
    # Act
    dataset = PandasTimeseriesDataset(
        data=sample_data,
...
def test_feature_names_property(dataset: PandasTimeseriesDataset):
    # Arrange
...
def test_index_property(dataset: PandasTimeseriesDataset, sample_data: pd.DataFrame) -> None:
    # Arrange
...
def test_data_property(dataset: PandasTimeseriesDataset, sample_data: pd.DataFrame) -> None:
    # Arrange
...
def test_get_window_basic_filtering(dataset: PandasTimeseriesDataset):
    # Arrange
...
def test_get_window_with_availability_filtering(dataset: PandasTimeseriesDataset):
    # Arrange
    start = datetime.fromisoformat("2023-01-01T10:00:00")
...
def test_get_window_duplicates_keep_latest(dataset: PandasTimeseriesDataset):
    # Arrange
...
def test_get_window_reindex_with_missing_timestamps():
    ...
    gapped_dataset = PandasTimeseriesDataset(
        data=data,
        sample_interval=timedelta(hours=1),
        timestamp_column="timestamp",
        available_at_column="available_at",
...
def test_from_parquet(tmp_path: Path):
    ...
    # Act
    dataset = PandasTimeseriesDataset.from_parquet(path=parquet_path)
