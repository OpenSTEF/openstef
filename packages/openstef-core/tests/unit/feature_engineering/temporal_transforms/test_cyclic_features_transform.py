...
"""Unit tests for the CyclicFeaturesTransform."""
...
from openstef_core.feature_engineering.temporal_transforms.cyclic_features_transform import CyclicFeaturesTransform
...
def test_cyclic_features_custom_features():
    ...
    transform = CyclicFeaturesTransform(included_features=["season", "month"])
    result = transform.transform(input_data)
...
def test_cyclic_features_no_features():
    ...
    # No features
    transform = CyclicFeaturesTransform(included_features=[])
    result = transform.transform(input_data)
