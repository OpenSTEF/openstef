...
def sample_dataset_no_tz() -> TimeSeriesDataset:
    ...
    """
    data = pd.DataFrame(
        {"radiation": [3600000, 7200000, 5400000]},
        index=pd.date_range("2025-06-01", periods=3, freq="h")
    )
    return TimeSeriesDataset(data, timedelta(hours=1))
...
@pytest.fixture
def basic_config() -> RadiationDerivedFeaturesConfig:
    """
    Create a basic configuration for RadiationDerivedFeatures.

    Returns:
        RadiationDerivedFeaturesConfig: Configuration with Amsterdam coordinates.
    """
    return RadiationDerivedFeaturesConfig(
        latitude=52.3676,
        longitude=4.9041,
...
def test_radiation_derived_features_config_initialization():
    """Test RadiationDerivedFeaturesConfig can be initialized properly."""
    # Act
    config = RadiationDerivedFeaturesConfig(latitude=52.0, longitude=5.0)

    # Assert
    assert config.latitude == 52.0
    assert config.longitude == 5.0
    assert config.included_features == ["dni", "gti"]
    assert config.surface_tilt == 34.0
    assert config.surface_azimuth == 180.0
...
def test_radiation_derived_features_config_validation(latitude: float, longitude: float, expected_valid: bool):
    """Test RadiationDerivedFeaturesConfig validation for latitude and longitude bounds."""
    if expected_valid:
        config = RadiationDerivedFeaturesConfig(latitude=latitude, longitude=longitude)
        assert config.latitude == latitude
        assert config.longitude == longitude
    else:
        with pytest.raises(ValueError):
            RadiationDerivedFeaturesConfig(latitude=latitude, longitude=longitude)


def test_radiation_derived_features_initialization(basic_config: RadiationDerivedFeaturesConfig):
    """Test RadiationDerivedFeatures can be initialized properly."""
    # Act
    transform = RadiationDerivedFeatures(config=basic_config)

    # Assert
    assert transform.config == basic_config
    assert transform.derived_features.empty
...
def test_fit_with_timezone_aware_dataset(
    ...
    basic_config: RadiationDerivedFeaturesConfig,
    ...
    with patch("pvlib.solarposition.get_solarposition", return_value=mock_solar_position), \
        ...
        # Assert
        assert not transform.derived_features.empty
        assert "dni" in transform.derived_features.columns
        assert "gti" in transform.derived_features.columns
        assert len(transform.derived_features) == len(sample_dataset.data)


def test_fit_raises_error_for_non_timezone_aware_dataset(
    sample_dataset_no_tz: TimeSeriesDataset,
    basic_config: RadiationDerivedFeaturesConfig
):
...
def test_fit_handles_missing_radiation_feature(
    sample_dataset_no_radiation: TimeSeriesDataset,
    basic_config: RadiationDerivedFeaturesConfig
):
    ...
    with patch("openstef_core.feature_engineering.weather_transforms.radiation_derived_features.logger") as mock_logger:
        transform.fit(sample_dataset_no_radiation)
...
@pytest.mark.parametrize("included_features,expected_columns", [
    (["dni"], ["dni"]),
    (["gti"], ["gti"]),
    (["dni", "gti"], ["dni", "gti"]),
])
def test_fit_with_different_feature_combinations(
    ...
    with patch("pvlib.solarposition.get_solarposition", return_value=mock_solar_position), \
        ...
        # Assert
        assert list(transform.derived_features.columns) == expected_columns


def test_calculate_dni_method(basic_config: RadiationDerivedFeaturesConfig):
    ...
    with patch("pvlib.irradiance.dni") as mock_dni:
        ...
        # Act
        result = transform._calculate_dni(solar_position, clearsky_radiation, ghi)
...
def test_calculate_gti_method(basic_config: RadiationDerivedFeaturesConfig):
    ...
    with patch("pvlib.irradiance.get_total_irradiance") as mock_gti:
        ...
        # Act
        result = transform._calculate_gti(
            solar_position, dni, ghi, clearsky_radiation,
            surface_tilt=34.0, surface_azimuth=180.0
        )
...
def test_transform_adds_derived_features(
    ...
    transform.derived_features = mock_derived_features
...
def test_transform_with_empty_derived_features(
    ...
    # Arrange
    transform = RadiationDerivedFeatures(config=basic_config)
    # Leave derived_features as empty DataFrame
...
def test_radiation_unit_conversion(
    ...
    with patch("pvlib.solarposition.get_solarposition"), \
...
@pytest.mark.parametrize("surface_tilt,surface_azimuth", [
...
def test_dni_fillna_behavior(basic_config: RadiationDerivedFeaturesConfig):
