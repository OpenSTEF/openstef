...
from openstef_meta.models.rules_forecaster import (
    RulesForecaster,
...
@pytest.fixture
def base_config() -> RulesForecasterConfig:
    """Base configuration for Rules forecaster tests."""

    params = RulesForecasterHyperParams()
...
def test_rules_forecaster_predict_not_fitted_raises_error(
    sample_forecast_input_dataset: ForecastInputDataset,
    base_config: RulesForecasterConfig,
):
    ...
    # Act & Assert
    with pytest.raises(NotFittedError, match="RulesForecaster"):
        forecaster.predict(sample_forecast_input_dataset)


def test_rules_forecaster_with_sample_weights(
    ...
    # Arrange
    forecaster_with_weights = RulesForecaster(config=base_config)
    ...
    forecaster_without_weights = RulesForecaster(config=base_config)
...
def test_rules_forecaster_with_additional_features(
    sample_forecast_input_dataset: ForecastInputDataset,
    base_config: RulesForecasterConfig,
):
