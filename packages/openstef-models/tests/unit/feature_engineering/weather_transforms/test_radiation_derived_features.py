...
"""Unit tests for the RadiationDerivedFeaturesTransform."""
...
from openstef_models.feature_engineering.weather_transforms import RadiationDerivedFeaturesTransform
...
@pytest.fixture
def basic_transform() -> RadiationDerivedFeaturesTransform:
    """Create a basic RadiationDerivedFeaturesTransform instance."""
    return RadiationDerivedFeaturesTransform(
...
def test_transform_raises_error_for_non_timezone_aware_dataset(
    sample_dataset_no_tz: TimeSeriesDataset, basic_transform: RadiationDerivedFeaturesTransform
):
...
def test_transform_raises_error_for_missing_radiation_column(
    sample_dataset_no_radiation: TimeSeriesDataset, basic_transform: RadiationDerivedFeaturesTransform
):
    """Test that transform raises MissingColumnsError when radiation column is missing."""
    # Act & Assert
    with pytest.raises(MissingColumnsError):
        basic_transform.transform(sample_dataset_no_radiation)
...
def test_transform_raises_error_when_pvlib_not_available(sample_dataset: TimeSeriesDataset):
    ...
    with (
        ...
        pytest.raises(MissingExtraError, match="feature_engineering"),
    ):
...
def test_transform_preserves_original_data_and_metadata(
    sample_dataset: TimeSeriesDataset,
    basic_transform: RadiationDerivedFeaturesTransform,
):
...
def test_transform_with_different_surface_orientations(
    ...
    """Test RadiationDerivedFeaturesTransform with different surface orientations."""
    # Arrange
    transform = RadiationDerivedFeaturesTransform(
        coordinate=Coordinate(latitude=Latitude(52.0), longitude=Longitude(5.0)),
...
def test_transform_with_missing_custom_radiation_column():
    """Test transform raises error when custom radiation column is missing."""
    # Arrange
    data = pd.DataFrame(
...
def test_pvlib_integration_different_locations(latitude: float, longitude: float, timezone: str):
    """Test RadiationDerivedFeaturesTransform with real pvlib calls across different locations."""
    # Arrange
    data = pd.DataFrame(
...
def test_pvlib_integration_surface_orientations():
    ...
    south_facing = RadiationDerivedFeaturesTransform(
        coordinate=Coordinate(latitude=Latitude(52.0), longitude=Longitude(5.0)),
        surface_tilt=30.0,
        surface_azimuth=180.0,
    )
    east_facing = RadiationDerivedFeaturesTransform(
        coordinate=Coordinate(latitude=Latitude(52.0), longitude=Longitude(5.0)),
        surface_tilt=30.0,
