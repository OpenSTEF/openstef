...
def test_xgboost_forecaster__supports_batching(base_config: XGBoostForecaster):
    """XGBoost declares a real (non-default-loop) batch path."""
    assert base_config.supports_batching is True


def test_xgboost_forecaster__predict_batch_matches_loop(
    sample_forecast_input_dataset: ForecastInputDataset,
    base_config: XGBoostForecaster,
):
    """predict_batch must be row-for-row equivalent to looping predict."""
    # Arrange
    forecaster = base_config.model_copy(deep=True)
    forecaster.fit(sample_forecast_input_dataset)
    items = [sample_forecast_input_dataset, sample_forecast_input_dataset]

    # Act
    batch = forecaster.predict_batch(items)
    looped = [forecaster.predict(item) for item in items]

    # Assert
    assert len(batch) == len(items)
    for batched, single in zip(batch, looped, strict=True):
        pd.testing.assert_frame_equal(batched.data, single.data)


def test_xgboost_forecaster__predict_batch_empty_returns_empty(
    sample_forecast_input_dataset: ForecastInputDataset,
    base_config: XGBoostForecaster,
):
...
def test_xgboost_forecaster__predict_batch_not_fitted_raises_error(
    sample_forecast_input_dataset: ForecastInputDataset,
    base_config: XGBoostForecaster,
):
    """predict_batch raises NotFittedError when called before fit()."""
    forecaster = base_config.model_copy(deep=True)

    with pytest.raises(NotFittedError, match="XGBoostForecaster"):
        forecaster.predict_batch([sample_forecast_input_dataset])
