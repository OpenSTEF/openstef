...
@pytest.fixture
def clipper() -> Clipper:
    return Clipper(column_names=["A", "B", "D"])


def test_feature_clipper_fit(clipper: Clipper, train_dataset: TimeSeriesDataset):
...
def test_feature_clipper_transform(clipper: Clipper, train_dataset: TimeSeriesDataset, test_dataset: TimeSeriesDataset):
...
def test_feature_clipper_invalid_column(train_dataset: TimeSeriesDataset):
    ...
    clipper_with_invalid_column = Clipper(column_names=["E"])
...
def test_feature_clipper_transform_without_fit(clipper: Clipper, test_dataset: TimeSeriesDataset):
    """Test behavior when transform is called without fitting."""
    clipper = Clipper(column_names=["A", "B"])
...
def test_clipping_transform__state_roundtrip(train_dataset: TimeSeriesDataset, test_dataset: TimeSeriesDataset):
    ...
    # Arrange
    original_transform = Clipper(column_names=["A", "B"])
    original_transform.fit(train_dataset)

    # Act
    state = original_transform.to_state()
    restored_transform = Clipper(column_names=["A", "B"])
