...
def test_imputation_transform__state_roundtrip(sample_dataset: TimeSeriesDataset):
    ...
    # Arrange
    original_transform = ImputationTransform(imputation_strategy="mean", columns={"radiation", "temperature"})
    original_transform.fit(sample_dataset)

    # Act
    state = original_transform.to_state()
    restored_transform = ImputationTransform(imputation_strategy="mean")
