...
from openstef_models.transforms.general import RemoveEmptyColumnsTransform
...
def test_removes_only_specified_empty_columns(sample_dataset_with_empty_columns: TimeSeriesDataset):
    ...
    # Arrange
    transform = RemoveEmptyColumnsTransform(columns={"empty_col1", "radiation"})
...
def test_custom_missing_value_placeholder():
    ...
    transform = RemoveEmptyColumnsTransform(missing_value=-999.0)
...
def test_transform_not_fitted_error():
    ...
    # Act & Assert
    with pytest.raises(
        TransformNotFittedError, match="The transform 'RemoveEmptyColumnsTransform' has not been fitted"
    ):
...
def test_nonexistent_columns_are_ignored():
    ...
    dataset = TimeSeriesDataset(data, timedelta(hours=1))
    transform = RemoveEmptyColumnsTransform(columns={"nonexistent", "empty_col"})
...
def test_remove_empty_columns_transform__state_roundtrip(sample_dataset_with_empty_columns: TimeSeriesDataset):
    ...
    # Arrange
    original_transform = RemoveEmptyColumnsTransform(columns={"empty_col1", "radiation", "temperature"})
    original_transform.fit(sample_dataset_with_empty_columns)

    # Act
    state = original_transform.to_state()
    restored_transform = RemoveEmptyColumnsTransform()
