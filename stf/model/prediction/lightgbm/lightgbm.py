...
import stf.feature_engineering.general as feature_engineering
from stf.model.prediction.prediction import AbstractPredictionModel


class LGBPredictionModel(AbstractPredictionModel):
    def __init__(
    ...
    def make_forecast(self, forcast_input_data):
        ...
        """
        self.logger.info("Use LightGBM model to make a forecast")

        forecast_request = lgb.Dataset(forcast_input_data)

        model_forecast = self.model.predict(
            forecast_request, num_iteration=self.model.best_iteration
        )
