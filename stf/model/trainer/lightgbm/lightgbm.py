...
class LGBModelTrainer(AbstractModelTrainer):
    def __init__(self, pj):
        ...
        self._lgb_hyper_parameter_keys = [
            # "silent",
    ...
    def train(
        ...
        Returns:
            lightgbm.Booster: this object is also stored in the self.trained_model
            attribute.
        ...
        def custom_eval(preds, dset):
            return "MAE", metrics.mae(dset.get_label(), preds), False

        # get the lgb (hyper) parameters
        params = {k: self.hyper_parameters[k] for k in self._lgb_hyper_parameter_keys}
    ...
    def training_period_objective(
        ...
        Args:
            ...
            error_function (callable): Function to calculate the error metric to be
                optimized, preferably one from stf.model.metrics.
