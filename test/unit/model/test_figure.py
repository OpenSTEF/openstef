...
class TestFigure(BaseTestCase):
    ...
    def test_plot_data_series(self):
        ...
        fig = plot_data_series([self.train_data, self.data], [self.train_data.rename(columns={'load': 'forecast'}), self.data.rename(
            columns={'load': 'forecast'})])
    ...
    def test_plot_feature_importtance(self):
        feature_importance = pd.DataFrame(data={'gain': [0.25, 0.25, 0.5], 'weight': [
                                          0.5, 0.2, 0.3]}, index=['a', 'b', 'c'])
