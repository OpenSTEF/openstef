...
class TestPreprocessing(BaseTestCase):
    def test_replace_repeated_values_with_nan(self):
        ...
        df = pd.DataFrame({"Column1": data_two_repeated_sequences})

        # Act
        df_repeated_sequences_below_threshold = (
            preprocessing.replace_repeated_values_with_nan(df, 8, "Column1")
        )
        ...
        self.assertEqual(df_one_repeated_sequence.isna().values.sum(), 4)
        self.assertEqual(df_two_repeated_sequences.isna().values.sum(), 7)

    def test_replace_repeated_values_with_nan_ignore_no_repetition(self):
        # Arrange
